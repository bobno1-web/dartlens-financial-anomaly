...
FINAL_RATIO_COLUMNS = [
    "비율명", "그룹", "산식", "삼성전자 값", "산업 평균", "산업 중앙값", "산업 p25", "산업 p75", "IQR",
    "중앙값 대비 비율차이(%)", "중앙값 대비 차이(%p·값)", "robust_z", "percentile", "판정",
    "benchmark_quality", "해석 비고", "감사 관점 코멘트", "사용 계정", "계산 가능 peer 수",
    "peer 후보 수", "CFS 성공 peer 수", "CFS 실패 peer 수", "source reference", "판정 사유(reason)",
]
...
def _write_final_ratio_sheet(wb, ws, comp_rows, *, ratio_fmt, pct_fmt, z_fmt, int_fmt,
                             pp_fmt, turn_fmt, wrap, header_fmt, label_fmts, label_ko):
    ws.merge_range(0, 0, 0, len(FINAL_RATIO_COLUMNS) - 1,
                   "HIGH=산업 대비 높음 / LOW=산업 대비 낮음 (좋음·나쁨이 아니라 산업 대비 상대적 위치). "
                   "판정은 median/IQR 기준, 평균은 참고값. '중앙값 대비 비율차이(%)'는 (값−중앙값)/|중앙값|, "
                   "'중앙값 대비 차이(%p·값)'는 값−중앙값(비율은 %p, 회전율은 값 차이). 둘·percentile·robust_z를 함께 해석.",
                   wrap)
    for c, h in enumerate(FINAL_RATIO_COLUMNS):
        ws.write(1, c, h, header_fmt)
    widths = [16, 10, 26, 14, 14, 14, 14, 14, 12, 16, 18, 10, 10, 14, 16, 40, 46, 34, 14, 12, 14, 14, 60, 46]
    for c, w in enumerate(widths):
        ws.set_column(c, c, w)
    r = 2
    for row in comp_rows:
        st = row["stats"]
        label = row["label"]
        pp_col_fmt = pp_fmt if row.get("unit") == "pp" else turn_fmt
        vals = [
            row["ratio"], row["group"], row["formula"],
            (row["target_value"], ratio_fmt), (st["mean"], ratio_fmt), (st["median"], ratio_fmt),
            (st["p25"], ratio_fmt), (st["p75"], ratio_fmt), (st["iqr"], ratio_fmt),
            (row["deviation_rate"], pct_fmt), (row.get("deviation_pp_display"), pp_col_fmt),
            (row["robust_z"], z_fmt), (row["percentile"], z_fmt),
            (label_ko.get(label, label), label_fmts.get(label)),
            row["benchmark_quality"], (row.get("interpret_note", ""), wrap), (row["audit_comment"], wrap),
            row["accounts_used"],
            (st["n_companies"], int_fmt), (row["peer_candidates"], int_fmt),
            (row["cfs_success"], int_fmt), (row["cfs_fail"], int_fmt),
            (row["source_reference"], wrap), (row["reason"], wrap),
        ]
        for c, cell in enumerate(vals):
            if isinstance(cell, tuple):
                ...
                if isinstance(v, str):
                    ...
                    ws.write_number(r, c, float(v), fmt) if fmt else ws.write_number(r, c, float(v))
            else:
                ws.write(r, c, cell)
        r += 1
    ws.freeze_panes(2, 0)
...
def build_final_report_workbook(path: Path, *, target: dict, target_cfs_rows: list[dict],
                                ...
                                meta: dict) -> Path:
    """최종 사용자용 산업대비 이상징후 리포트(9시트, 전부 한글)."""
    ...
    readme = [
        ...
        ("전부 NORMAL 해석", f"15개 비율이 모두 NORMAL이라는 것은 안전이 확정됐다는 의미가 아니라, 현재 induty_code {target['induty_code']} peer universe와 median/IQR fence 기준에서 이상치로 분류되지 않았다는 뜻입니다. 추가 확인이 여전히 필요할 수 있습니다."),
        ...
        ("leave-one-out", "삼성전자는 자기 자신의 benchmark 계산에서 제외했습니다(비교 대상값으로만 사용)."),
        ("매핑 정책", "매출채권·매입채무는 순수 계정 기준이며, '및기타채권/및기타채무' fallback은 사용하지 않았습니다."),
        ("통계 기준", "평균은 참고값이며 판정은 median/IQR 중심입니다. winsorized 평균은 참고값입니다."),
        ("비교가능성 한계", "삼성전자는 복합 대형회사이므로 induty_code 기반 peer benchmark에는 비교가능성 한계가 있습니다."),
    ...
    for sheet_name in _SHEET_ORDER:
        ...
        _write_final_ratio_sheet(wb, ws, by_group.get(sheet_name, []),
                                 ratio_fmt=ratio_fmt, pct_fmt=pct_fmt, z_fmt=z_fmt, int_fmt=int_fmt,
                                 pp_fmt=pp_fmt, turn_fmt=turn_fmt, wrap=wrap, header_fmt=bold,
                                 label_fmts=label_fmts, label_ko=cmp.LABEL_KO)
...
def _methodology_loop3(ws, wb, target, meta, n_cand, n_succ, n_fail, bold, wrap):
    ...
    lines = [
        ...
        ("leave-one-out", "삼성전자는 모든 비율의 benchmark 계산에서 제외(비교 대상값으로만 유지)."),
        ...
        ("전부 NORMAL 해석", "15개 비율 모두 NORMAL은 안전 확정이 아니라 현재 peer universe·IQR fence 기준상 이상치로 분류되지 않았다는 의미. 추가 확인이 필요할 수 있음."),
        ...
        ("비교가능성 한계", "삼성전자는 복합 대형회사로 단일 induty_code peer benchmark와 비교가능성 한계가 있음."),
...
def build_benchmark_debug_workbook(path: Path, *, target: dict, comparison_rows: list[dict],
                                   ...
                                   target_trace: list[dict], meta: dict) -> Path:
    """Loop 3 검증용 benchmark debug(8시트, 전부 한글)."""
    ...
    readme = [
        ...
        ("목적", "비율별 benchmark pool 구성·통계·삼성전자 comparison·label 산정근거·품질을 사람이 검증."),
        ("pool 원칙", "비율별 계산 가능 peer만 pool 포함. 삼성전자는 leave-one-out으로 제외(01 시트에서 포함/제외·사유 확인)."),
    ...
    # 03_삼성전자_comparison
    ws = wb.add_worksheet("03_삼성전자_comparison")
    crows = []
    for pd in pool_details:
    ...
    _add_table(wb, ws, ["비율명", "삼성전자값", "median", "p25", "p75", "iqr", "상단fence(p75+k·iqr)",
    ...
    for pd in pool_details:
        c = cmp_by_ratio.get(pd["ratio"], {})
        basis = (f"삼성값={_fmt(c.get('target_value'))}, 상단fence={_fmt(pd.get('upper_fence'))}, "
    ...
    # 07_source_trace_샘플
    ws = wb.add_worksheet("07_source_trace_샘플")
    ws.write(0, 0, "삼성전자 15개 비율 입력값 — 각 계정은 전체_CFS_Long(별도 파일)에서 (기업코드+계정ID+rcept_no)로 raw까지 추적", note)
