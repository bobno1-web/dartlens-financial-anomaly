...
FINAL_RATIO_COLUMNS = [
    "비율명", "그룹", "산식", "대상회사 값", "산업 평균", "산업 중앙값", "산업 p25", "산업 p75", "IQR",
...
def build_final_report_workbook(path: Path, *, target: dict, target_cfs_rows: list[dict],
                                ...
                                meta: dict, sparse_comparison: list[dict] | None = None) -> Path:
...
def build_benchmark_debug_workbook(path: Path, *, target: dict, comparison_rows: list[dict],
    ...
    # 07_source_trace_샘플
    ws = wb.add_worksheet("07_source_trace_샘플")
    ws.write(0, 0, "대상 회사 15개 비율 입력값 — 각 계정은 전체_CFS_Long(별도 파일)에서 (기업코드+계정ID+rcept_no)로 raw까지 추적", note)
